...
---
### create()
Не должен возвращать Optional!

Создание либо:
- создаёт и возвращает объект ✅
- либо кидает ошибку ❌

Возврат `None` для create() — плохой контракт. Это техническая ошибка, не бизнес-ошибка

---
### delete()

`deleted = True / ORM` — если удалено

`deleted = False / None` — если такой категории нет
...
- сервис работает в строгом режиме
- “отсутствие сущности” считается ошибкой
- handler поймает NotFoundError и покажет сообщение
...
- отсутствие сущности считается допустимым
