...
async def show_admin_menu(event: Message | CallbackQuery, user, admin) -> None:
    """Точка входа в админку (/admin)."""
    logger.info("[Admin] User %s opened admin menu", user.id)
    await send_or_edit(event, ADMIN_MENU_TEXT, markup=get_admin_menu_keyboard(admin), parse_mode="HTML")


@admin_menu_router.callback_query(F.data == ADMIN_EXIT_PREFIX)
