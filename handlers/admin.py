...
from keyboards.admin_kb import get_admin_menu_keyboard, get_back_to_admin_menu_keyboard
...
async def show_admin_menu(event: Message | CallbackQuery) -> None:
    text = (
        "🛡️ <b>Админ-панель</b>\n\n"
        "Выберите раздел управления:"
    )
    await send_or_edit(event, text, markup=get_admin_menu_keyboard(), parse_mode="HTML")
...
@admin_router.callback_query(F.data == "admin:disputes")
