...
def build_profile_stats_text() -> str:
    ...
    return (
        ...
        "• 💰 ?: ?\n"
        "• 💸 ?: ?\n\n"
        "Статистика будет доступна после подключения агрегатов по заявкам."
    )
...
def default_achievements() -> list[tuple[str, bool]]:
    ...
    return [  # пока на шару расставил True\False
        ("Первая аренда", False),
        ("5 арендованных товаров", False),
        ("Премиум статус", False),
    ]

def build_achievements_text() -> str:
    """Сформировать текст экрана достижений"""
    # TODO: ? "Достижения пока готовятся. Совершайте заявки — мы обязательно покажем ваш прогресс здесь."
    ...
    if not has_achievements:
        achievements_message += "\nУ вас пока нет достижений.\nСовершайте заявки, чтобы их получить!"
...
def build_privacy_policy_text() -> str:
    ...
    return (
        ...
        "- Email (по желанию)\n"
        "- Информацию о товарах, которые вы выбираете\n"
        "- Местоположение для поиска оборудования (только город/район)\n\n"

        "🛡 <b>Как мы используем ваши данные:</b>\n"
        "- Для обеспечения работы сервиса аренды\n"
        "- Для обработки заявок и связи менеджера с клиентом\n"
