...
def build_start_date_keyboard(item_id: int, days_ahead: int = START_DATE_DAYS_AHEAD) -> InlineKeyboardMarkup:
    ...
    rows = [[InlineKeyboardButton(text="📅 Выберите дату начала аренды:", callback_data=IGNORE_CB)]]
...
def build_rent_cancel_keyboard(item_id: int | None) -> InlineKeyboardMarkup:
    ...
    if item_id is not None:
        rows.append([InlineKeyboardButton(text="🔙 Назад к объявлению", callback_data=f"{ITEM_DETAILS}{item_id}")])
...
def build_rent_success_keyboard() -> InlineKeyboardMarkup:
    """Собрать клавиатуру после успешной отправки запроса аренды"""
