...
Репозиторий товаров каталога компании, который создаётся и обновляется администраторами/менеджерами.
...
> `create/update/delete`
> 
> `update`- Принимает `UserUpdate | UserAdminUpdate`.
...
> `create` - Создаёт запись о действии администратора.
...
> `create` - использует `rental_data.model_dump()`

> `try_update_status` - Атомарно меняет статус заявки, если текущий статус равен `expected_status`
...
> `create/update/delete`
> 
> `ReviewCreateInternal/ReviewUpdate`
> 
> `create` использует `review_data.model_dump()`
...
> `create/update/delete`
> 
>`SupportTicketCreateInternal/SupportTicketAdminUpdate`
> 
> `create` - использует `ticket_data.model_dump()`
...
- `Model` (конкретный ORM-объект — для успешного `create`)
- `list[Model]` — для выборок;
